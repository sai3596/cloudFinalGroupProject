...
def login():
    ...
    if request.method == 'POST' and str(request.form['username']) != "" and str(request.form['password']) != "":
        ...
        result = executeselectquery(
            "SELECT firstname,lastname,email  FROM users WHERE username  ='" + username + "' AND password_hash = '" + password + "'")
        if (result.shape[0] != 0):
...
def registration():
    ...
    if request.method == 'POST' and str(request.form['username']) != "" and str(request.form['password']) != "" and str(
        ...
        if (result.shape[0] != 0):
            ...
            result1 = cursor.execute(
                """INSERT INTO users (username, password_hash, firstname, lastname, email) values (%s, %s, %s, %s, %s)""",
                (username, password, firstname, lastname, email))
            conn.commit()
            cursor.close()
            conn.close()
            result2 = executeselectquery(
                "SELECT firstname,lastname,email  FROM users WHERE username  = '" + username + "' AND password_hash ='" + password + "'")
...
def storeuploadedhouseholdfile():
    ...
    if request.method == 'POST':  # check if the method is post
        ...
        if check_file_extension(f.filename):
            f.save(os.path.join(app.config['Upload_folder_HouseHolds'],
                                secure_filename(f.filename)))  # this will secure the file
            readCSVandloaddata(os.path.join(app.config['Upload_folder_HouseHolds'], secure_filename(f.filename)),
                               "households");
...
def storeuploadedProductfile():
    ...
    if request.method == 'POST':  # check if the method is post
        f = request.files['file']
        if check_file_extension(f.filename):
            f.save(os.path.join(app.config['Upload_folder_Products'],
                                secure_filename(f.filename)))  # this will secure the file
            readCSVandloaddata(os.path.join(app.config['Upload_folder_Products'], secure_filename(f.filename)),
                               "products");
            message = 'file uploaded successfully'  # Display thsi message after uploading
        else:
            message = 'The file extension is not allowed'
...
def storeuploadedTransactionfile():
    ...
    if request.method == 'POST':  # check if the method is post
        ...
        if check_file_extension(f.filename):
            f.save(os.path.join(app.config['Upload_folder_Transactions'],
                                secure_filename(f.filename)))  # this will secure the file
            readCSVandloaddata(os.path.join(app.config['Upload_folder_Transactions'], secure_filename(f.filename)),
                               "transactions");
            message = 'file uploaded successfully'  # Display thsi message after uploading
        else:
            message = 'The file extension is not allowed'
